...
function getDate() {
  ...
  return util.format("%s %s %s:%s:%s", months[month], day, hours, minutes, seconds);
}
...
var Transport = {
  UDP: function(message, severity) {
    ...
    var buf = new Buffer('<' + (this.facility * 8 + severity) + '>' +
  ...
  file: (function() {
    ...
    switch(require('os').type()) {
      ...
      // XXX browserid works on windows
      // we need something for windows here
      default:
...
function SysLogger(config) {
  ...
  this._logError = function(err, other) {
    if(err){
      console.error('Cannot log message via %s:%d', this.hostname, this.port);
    }
  }.bind(this);
  this.set(config);
  return this;
...
/**
 * Init function, takes a configuration object. If a hostname is
 * provided the transport is assumed to be Transport.UDP
 *
