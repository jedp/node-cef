...
var validatorForKey = require('./extensions').validatorForKey;

var CURRENT_CEF_VERSION = "0";
var requiredParams = ['vendor', 'product', 'version', 'signature', 'name', 'severity'];
...
Formatter.prototype = {
  ...
  formatExtensions: function formatExtensions(extensions) {
    ...
    var validator = null;
    var err = null;
    _.each(_.keys(extensions), function(key) {
      ...
      if (validator) {
        ...
        if (validator(value)) {
          ...
          err = new Error(util.format("Not a valid value for %s: %s", key, value));
          return err;
        }
      } else {
        err = new Error("Not a valid CEF or ArcSight key: " + key);
        return err;
      }
    }, this);
